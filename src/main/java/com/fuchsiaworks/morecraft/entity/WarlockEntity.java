...
public class WarlockEntity extends AbstractRaiderEntity implements IRangedAttackMob {
	...
	private static final DataParameter<Boolean> IS_DRINKING = EntityDataManager.createKey(WarlockEntity.class,
	...
	public WarlockEntity(EntityType<? extends WarlockEntity> typeIn, World worldIn) {
		super(typeIn, worldIn);
	}

	protected void registerGoals() {
		super.registerGoals();
		this.field_213694_bC = new NearestAttackableTargetExpiringGoal<>(this, AbstractRaiderEntity.class, true,
				(p_213693_1_) -> {
					return p_213693_1_ != null && this.isRaidActive() && p_213693_1_.getType() != Entities.WARLOCK;
				});
		this.field_213695_bD = new ToggleableNearestAttackableTargetGoal<>(this, PlayerEntity.class, 10, true, false,
	...
	protected SoundEvent getAmbientSound() {
		return Sounds.ENTITY_WARLOCK_AMBIENT;
	}

	protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
		return Sounds.ENTITY_WARLOCK_HURT;
	}

	protected SoundEvent getDeathSound() {
		return Sounds.ENTITY_WARLOCK_DEATH;
	}
	...
	public static AttributeModifierMap.MutableAttribute getEntityAttributes() {
		return MonsterEntity.func_234295_eP_().func_233815_a_(Attributes.field_233818_a_, 26.0D)
		...
		if (!this.world.isRemote && this.isAlive()) {
			...
			if (this.isDrinkingPotion()) {
				if (this.potionUseTimer-- <= 0) {
					...
					this.getAttribute(Attributes.field_233821_d_).removeModifier(MODIFIER);
				}
			} else {
				Potion potion = null;
				if (this.rand.nextFloat() < 0.15F && this.areEyesInFluid(FluidTags.WATER)
						&& !this.isPotionActive(Effects.WATER_BREATHING)) {
					...
					potion = Potions.HEALING;
						...
						&& !this.isPotionActive(Effects.SPEED) && this.getAttackTarget().getDistanceSq(this) > 121.0D) {
				...
				if (potion != null) {
					...
					if (!this.isSilent()) {
						this.world.playSound((PlayerEntity) null, this.getPosX(), this.getPosY(), this.getPosZ(),
								Sounds.ENTITY_WARLOCK_DRINK, this.getSoundCategory(), 1.0F,
								0.8F + this.rand.nextFloat() * 0.4F);
					}

					ModifiableAttributeInstance modifiableattributeinstance = this
							.getAttribute(Attributes.field_233821_d_);
					modifiableattributeinstance.removeModifier(MODIFIER);
					modifiableattributeinstance.func_233767_b_(MODIFIER);
				}
			}
	...
	public SoundEvent getRaidLossSound() {
		return Sounds.ENTITY_WARLOCK_CELEBRATE;
	}
		...
		if (id == 15) {
			for (int i = 0; i < this.rand.nextInt(35) + 10; ++i) {
				this.world.addParticle(
						ParticleTypes.WITCH/* Particles.WARLOCK */,
						this.getPosX() + this.rand.nextGaussian() * (double) 0.13F,
						this.getBoundingBox().maxY + 0.5D + this.rand.nextGaussian() * (double) 0.13F,
						this.getPosZ() + this.rand.nextGaussian() * (double) 0.13F, 0.0D, 0.0D, 0.0D);
			}
		} else {
	...
	public void attackEntityWithRangedAttack(LivingEntity target, float distanceFactor) {
		if (!this.isDrinkingPotion()) {
			...
			if (!this.isSilent()) {
				this.world.playSound((PlayerEntity) null, this.getPosX(), this.getPosY(), this.getPosZ(),
						Sounds.ENTITY_WARLOCK_THROW, this.getSoundCategory(), 1.0F,
						0.8F + this.rand.nextFloat() * 0.4F);
			}
